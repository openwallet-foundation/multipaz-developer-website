...
The **Marketplace** is where the ecosystem comes together. Checking out an age-restricted item (say, a
bottle of wine) requires proving you're 18+ **and** paying — and the Marketplace does both in a
**single OpenID4VP presentation**. This is the most
interesting server in Utopia, so we'll read its code.

## Try it

1. Open **`http://localhost:8100/marketplace/`** on the device. Pick a product and go to checkout.
2. The page calls the Marketplace's `/checkout` endpoint, gets back a DCQL query plus transaction data,
...
4. The Marketplace verifies the presentation, checks your age, runs the payment through UPay, and shows
   **approved** or **declined**.

:::note What you present in this codelab
The Marketplace's DCQL accepts several ID types — mDL, ISO photo ID, EU PID, Aadhaar — but the **mDL** is
...
The Marketplace mounts all the standard Multipaz verifier endpoints, then adds its storefront's
`/checkout`:
...
fun Application.configureRouting(environment: Deferred<ServerEnvironment>) {
    routing {
        ...
        post("/checkout") {
            marketplaceCheckout(call)
        }
    }
}
```

`/checkout` takes `{productId}` and looks the product up in its **server-side catalog** to get the
authoritative name, price, and age-restricted flag — the browser never sends these, so they can't be
tampered with. It then opens a transaction with the payment processor and returns the **DCQL query**
(the age-and-payment query for age-restricted items, payment-only otherwise) and **transaction data**
for the browser to hand straight to `multipazVerifyCredentials()`.
...
credential — so the Marketplace works for holders from different ecosystems.
...
logic** goes in a `VerifierAssistant`. The Marketplace registers one in `Main`:
...
A 20-year-old has `age_over_21 = false` yet is still over 18. The Marketplace treats `age_over_21` as a
...
during `/checkout`, moving money to the Marketplace's account through the payment processor:
...
In one OpenID4VP exchange, the Marketplace proved the customer is of age **and** collected payment,
